   ...
   public function importCreditNotes(Request $request)
{
    ...
    try {
        ...
        for ($i = 1; $i < count($rows); $i++) {
            ...
            if (CreditNote::where('credit_note_id', $row['billing document'])->exists()) {
                $duplicateCreditNotes[] = $row['billing document'];
                continue;
            }
           ...
           $invoiceDate = null;
            ...
            CreditNote::create([
                'credit_note_id' => $row['billing document'],
        ...
        if (!empty($duplicateCreditNotes)) {
            $msg .= " Skipped duplicates: " . implode(', ', $duplicateCreditNotes);
        }

        ActivitLogService::log('import', 'credit notes imported from file - ' . $fileName);
