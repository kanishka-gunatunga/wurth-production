...
class ExtraPaymentController extends Controller
{
   ...
   public function importExtraPayments(Request $request)
{
    $request->validate([
    ...
    try {
        ...
        for ($i = 1; $i < count($rows); $i++) {
            ...
            if (ExtraPayment::where('extra_payment_id', $row['billing document'])->exists()) {
                $duplicateExtraPayments[] = $row['billing document'];
                continue;
            }
           ...
           $invoiceDate = null;
            ...
            ExtraPayment::create([
                'extra_payment_id' => $row['billing document'],
                'customer_id' => $customer->customer_id,
                'amount' =>$amount,
                'adm_id' =>$row['sales territory'] ?? null,
            ]);
        ...
        if (!empty($duplicateExtraPayments)) {
            $msg .= " Skipped duplicates: " . implode(', ', $duplicateExtraPayments);
        }

        ActivitLogService::log('import', 'extra payments imported from file - ' . $fileName);
