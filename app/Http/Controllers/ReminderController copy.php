...
public function getUsersByLevel(Request $request, $level)
{
    ...
    // Rule 0: If TARGET level is 1 (Admin) or 7 (Finance), DO NOT filter by division.
    // (Because these users are global and don't belong to divisions)
    if (in_array((int)$level, [1, 7])) {
        // No extra filtering needed.
    }
    // Rule 1: If Current User is NOT Admin(1)/Finance(7) -> Limit to THEIR division
    elseif (!in_array($currentUserRole, [1, 7])) {
    ...
    // Rule 2: If Current User IS Admin(1)/Finance(7) -> Optional Filter
    else {
