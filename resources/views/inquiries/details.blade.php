...
<div class="main-wrapper">
    ...
    <div class="d-flex justify-content-between align-items-center header-with-button">
        ...
        <span class="slip-detail-text">
            ...
            @endphp
            <button id="inquiry-status-btn" class="{{ $statusClass }}">
                {{ $inquiry->status }}
            </button>
        </span>
    </div>
...
@section('footer-buttons')
    ...
    @if ($status === 'pending')
        <button class="red-action-btn-lg update-status-btn" data-id="{{ $inquiry->id }}"
            data-status="rejected">Reject</button>
        <button class="success-action-btn-lg update-status-btn" data-id="{{ $inquiry->id }}"
            data-status="sorted">Approve</button>
            ...
            fetch(url, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        const statusButton = document.getElementById('inquiry-status-btn');
                        ...
                        if (data.success && statusButton) {
                            const normalized = data.status.toLowerCase();
                            statusButton.textContent = data.status;
                            statusButton.className = statusMap[normalized] ?? 'grey-status-btn';
                        }
