 ...
 public function customers(Request $request)
{
    ...
    // --- RETURN VIEW ---
    return view('finance::customer.customers', [
        'customers' => $customers,
    ...
    { if($request->isMethod('get')){
        $adms = User::where('user_role', '6')->with('userDetails')->get();
        return view('finance::customer.add_new_customer',['adms' => $adms]);
    }
    ...
    $adms = User::where('user_role', '6')->with('userDetails')->get();
    return view('finance::customer.edit_customer', ['customer_details' => $customer_details,'adms' => $adms]);
    }
    if($request->isMethod('post')){
    ...
    if($request->isMethod('get')){
        $customer_details = Customers::where('id', $id)
            ...
            ->first();
    return view('finance::customer.view_customer', ['customer_details' => $customer_details]);
    }
