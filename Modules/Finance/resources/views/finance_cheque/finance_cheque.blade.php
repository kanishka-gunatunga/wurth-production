...
<div class="main-wrapper">
    ...
    <div class="row d-flex justify-content-between">
        ...
        <div class="col-lg-6 col-12 d-flex justify-content-lg-end gap-3 pe-5">
            <form id="searchForm" method="POST" action="{{ route('finance_cheque.search') }}">
                @csrf
    ...
    <div class="styled-tab-main">
        ...
        @if(!empty($filters))
        <form method="POST" action="{{ route('finance_cheque.export') }}">
        ...
        <div class="table-responsive">
            <table class="table custom-table-locked">
                ...
                <tbody>
                    ...
                    <tr class="clickable-row" data-href="{{ route('finance_cheque.show', $item['id']) }}" style="cursor:pointer;">
                        ...
                        <td class="sticky-column">
                            ...
                            <a href="{{ route('finance_cheque.download', $item['id']) }}"
...
<form method="POST" action="{{ route('finance_cheque.filter') }}">
    @csrf
        ...
        <div class="offcanvas-body">
            ...
            <div class="mt-5 filter-categories">
                ...
                <select id="filter-adm-name" name="adm_names[]" class="form-control select2" multiple>
                    @foreach ($data->pluck('adm_name')->unique() as $admName)
                    @if($admName)
            ...
            <div class="mt-5 filter-categories">
                ...
                <select id="filter-adm-id" name="adm_ids[]" class="form-control select2" multiple>
                    @foreach ($data->pluck('adm_number')->unique() as $admId)
                    <option value="{{ $admId }}"
            ...
            <div class="mt-5 filter-categories">
                ...
                <select id="filter-customer" name="customers[]" class="form-control select2" multiple>
                    @foreach ($data->pluck('customer_name')->unique() as $customer)
                    @if($customer)
...
<script>
    ...
    document.addEventListener('click', function(e) {
        ...
        // Detect status update buttons
        if (e.target.classList.contains('update-status')) {
            e.preventDefault();
            e.stopPropagation();

            currentButton = e.target;
            selectedStatus = currentButton.dataset.status; // ← correct status!
        ...
        if (e.target.id === "confirm-yes-btn") {
            ...
            // Send request
            fetch("{{ route('finance_cheque.update_status', '') }}/" + depositId, {
                    method: "POST",
                    headers: {
